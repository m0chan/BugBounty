...
app.controller('myCtrl', ['$filter', '$location', '$scope', '$http', function ($filter, $location, $scope, $http) {
	...
	var supportedCarriers : string[] = ["mojio", "rogers", "telus", "bell", "tmobile",
										"vivint", "metropcs", "assurant", "att", "telcel",
										"tmcz", "tmpl", "tdg", "tmat", "optus","telefonica",
										"bosch", "tracfone"];

	var supportedLanguages : string[] =["en", "de", "cs", "es", "fr", "pl", "jp"];
	...
	function isBrandAndLanguageValid(brand : string , lan : string){
	...
	function setLanguage(lan : string , brand : string){
		switch(brand){
			case "telus" :
			case "rogers":
			case "bell":
	...
	function setConfigurations(brand : string){
		switch(brand) {
			case "mojio":
	...
	var segmentProxyUrl: {vehicleCompatibility: string, vehicleLookup: string} = {
	...
	function getBackendUrl(backendUrl: string) {
	...
	function createBrandExtention(brand: string) : string{
		var query = "?brand=";
